...
-) With no option:
docker run -t -p 8065:8065 -p 8060:8060 fiware/iotbroker:internal-standalone-dev

-) With options (the same contained in the confman.conf.default and iotbroker.conf.default)

docker run -t -p 8065:8065 -p 8060:8060 fiware/iotdiscovery:standalone-dev -p <iotbroker_key>="<value>" -p <confman_key>="<value>" [-p ...]
